...
+ (NSDictionary *)parseReadDataFromCharacteristic:(CBCharacteristic *)characteristic{
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"e49a26c1-f69a-11e8-8eb2-f2801f1b9fd1"]]) {
        
//        NSLog(@"characteristic.value11111111");
//        NSLog(@"%@",characteristic.value);
        //读取参数
        return [self parseReadConfigData:characteristic];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"e49a24c1-f69a-11e8-8eb2-f2801f1b9fd1"]]){
        NSLog(@"characteristic.value22222222");
        //设置数据
...
//    }
    //数据域长度
    NSString *function = [content substringWithRange:NSMakeRange(6, 2)];
...
//        return nil;
//    }
    NSLog(@"parseSetConfigData--3--%@",content);
    NSDictionary *returnData = nil;
    mk_taskOperationID operationID = mk_defaultTaskOperationID;
    NSString *function = [content substringWithRange:NSMakeRange(6, 2)];
...
//        //查看鉴权状态
//        operationID = mk_queryAuthState;
//    }else if ([function isEqualToString:@"01"]){
//        //鉴权
//        operationID = mk_deviceBind;
//    }
