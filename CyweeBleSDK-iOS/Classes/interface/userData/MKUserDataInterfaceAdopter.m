...
//将步数同步的参数转换成字符串
+ (NSString *)formatSyncStepsCommandString:(int)year
                                     month:(int)month
                                       day:(int)day
                                      type:(int)type
{
    NSMutableArray<NSNumber *> *dataList = [NSMutableArray array];
    Byte isRecordByte = type == 1 ? 0x01 : 0x00;
    [dataList addObject:@(isRecordByte)];

    // 年
    NSArray<NSNumber *> *yearData = [mk_fitpoloAdopter convert:year byteType:mk_word]; // ByteType.WORD 假设为 2
    [dataList addObjectsFromArray:yearData];
    ...
    // 假设 order.getOrderHeader() 返回一个 Byte 类型的值
    ...
    // 现在 dataBytes 是最终的字节数组
    NSString *commandString = [mk_fitpoloAdopter hexStringFromData:dataBytes];
    NSLog(@"发送指令===: %@", commandString);
    return commandString;
}

//将数据同步类型参数转换成字符串
+ (NSString *)formatDataPushCommandString:(int)orderType
                                     year:(int)year
                                    month:(int)month
                                      day:(int)day
{
    NSMutableArray<NSNumber *> *dataList = [NSMutableArray array];
    [dataList addObject:@(0x01)];//type
    // 年
    NSArray<NSNumber *> *yearData = [mk_fitpoloAdopter convert:year byteType:mk_word]; //ByteType.WORD为2
    [dataList addObjectsFromArray:yearData];
    ...
    [byteList addObject:@((Byte)(orderType & 0xFF))]; //orderType
    [byteList addObject:@(dataLength)];
